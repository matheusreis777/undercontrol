...
import { DownloadStoragePipe } from '../../directives/pipes/download-storage/download-storage.pipe';
import { IconDirective } from '../../directives/icon/icont.directive';
  ...
  private createForm() {
    this.config?.forEach(control => {
      if (control.type.field !== eDynamicField.DIVIDER) {
        this.form.addControl(control.name, new FormControl({ value: control.initialValue !== undefined ? control.initialValue : null, disabled: control.disabled }, control.validations));
      }
    });
  }
}
